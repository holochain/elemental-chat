import {
  ...
  export const defaultConfig = {
    trycpAddresses: [
      ...
      // "172.26.21.65:9000", //bekah (5zmks2xs8r2gbazho8du7ic0rgp57bd03i9rcev4wtqeievexi)
    ...
    instances: 50, // Instances per conductor
  ...
  const parseStateDump = ([
    unused,
    stateDumpRelevant
  ]: StateDump): StateDumpRelevant => {
  ...
  const waitAllPeers = async (
    ...
    let now = Date.now()
    console.log(
      `Start waiting for peer stores at ${new Date(now).toLocaleString('en-US')}`
    )
    ...
    for (const playerIdx in allPlayers) {
      for (const agentIdx in playerAgents[playerIdx]) {
        ...
        while (true) {
          const stateDumpRes = await player
            .adminWs()
            .dumpState({ cell_id: agent.cell.cellId })
          console.log('state dump:', stateDumpRes)
          const stateDump = parseStateDump(stateDumpRes)
          console.log(
            `waiting for all agents are present in peer store of player #${playerIdx} agent #${agentIdx}`,
            stateDump
          )
    ...
    console.log(
      `Finished waiting for peers at ${new Date(endWaitPeers).toLocaleString(
        'en-US'
      )}`
    )
  ...
  const waitActivePeers = async (
    ...
    console.log(
      `Start waiting for peer stores at ${new Date(startWait).toLocaleString(
        'en-US'
      )}`
    )
    let conductors = {}
    ...
    for (const [playerIdx, param] of Object.entries(conductors)) {
      while (true) {
        const stateDumpRes = await allPlayers[playerIdx]
          .adminWs()
          .dumpState(param)
        const stateDump = parseStateDump(stateDumpRes)
        ...
        console.log(
          `got ${
            stateDump.numPeers
          } of ${needed} (${percentNeeded}% of ${totalPeers}) of peers to be present in peer store of player #${playerIdx}, ${time2text(
            Date.now() - startWait
          )} so far`,
          stateDump
        )
  ...
  const _activateAgents = async (
    ...
    console.log(
      `Start calling refresh chatter for ${
        activeAgents.length
  ...
  const _waitAgentsActivated = async (activeAgents: Agents): Promise<number> => {
    let startWait = Date.now()
    console.log(
      `Start find agents at ${new Date(startWait).toLocaleString('en-US')}`
    )
    // wait for all active agents to see all other active agents:
    for (const agentIdx in activeAgents) {
      while (true) {
        const stats = await activeAgents[agentIdx].cell.call(
          'chat',
          'agent_stats',
          null
        )
        console.log(
          `waiting for #${agentIdx}'s agent_stats to show ${
            activeAgents.length
          } as active (${time2text(Date.now() - startWait)} so far), got:`,
          stats
        )
    ...
    console.log(
      `End find agents at ${new Date(endFindAgents).toLocaleString('en-US')}`
    )
  ...
  const doListMessages = async (
    ...
    const counts: Array<number> = await Promise.all(
      activeAgents.map(async agent => {
        const r = await agent.cell.call('chat', 'list_messages', {
  ...
  const setup = async (
    ...
    const installation: InstallAgentsHapps = _.times(config.instances, () => [
      [config.dnaSource]
    ])
    ...
    for (let i = 0; i < config.conductors; i++) {
      ...
      network.transport_pool[0].proxy_config.proxy_url =
        config.proxys[i % config.proxyCount]
    ...
    if (config.trycpAddresses.length == 0) {
      ...
      while (allPlayers.length / config.conductors < config.nodes) {
        if (i >= config.trycpAddresses.length) {
          throw new Error(
            `ran out of trycp addresses after contacting ${allPlayers.length /
              config.conductors} nodes`
          )
        }
        let players: Player[]
        try {
          console.log(
            'Starting players at trycp server:',
            config.trycpAddresses[i]
          )
          players = await s.players(
            conductorConfigsArray,
            false,
            config.trycpAddresses[i]
          )
    ...
    const playerAgents: PlayerAgents = await Promise.all(
      allPlayers.map(async (player, i) => {
        ...
        const memProofArray = Object.keys(memProofJSON)
            .map(function(key) {
                return memProofJSON[key];
            });
        const agents = await installAgents(player, agentNames, i, memProofArray,'uhCAkRHEsXSAebzKJtPsLY1XcNePAFIieFBtz2ATanlokxnSC1Kkz')
    ...
    if (local) {
      let now = Date.now()
      console.log(
        `Calling share all nodes ${new Date(now).toLocaleString('en-US')}`
      )
      await s.shareAllNodes(allPlayers)
      console.log(
        `Finished share all nodes ${new Date(Date.now()).toLocaleString('en-US')}`
      )
    }
    ...
    const createChannelResult = await playerAgents[0][0].cell.call(
      'chat',
      'create_channel',
  ...
  const sendConcurrently = async (
    ...
    for (let i = 0; i < messagesToSend; i++) {
      messagePromises[i] = send(
        i,
        agents[i % agents.length].cell,
        channel,
        signal
      )
    }
  ...
  const gossipTrial = async (
    ...
    while (true) {
      ...
      try {
        justReceived = (
          await receivingCell.call('chat', 'list_messages', {
            channel: channel.entry,
            active_chatter: false,
            chunk: { start: 0, end: 1 }
          })
      ...
      if (received !== justReceived) {
        ...
        console.log(
          `After ${Date.now() - start}ms, receiver got ${received} messages`
        )
  ...
  const signalTrial = async (
    ...
    const allReceipts = new Promise<number | undefined>(
      (resolve, reject) => (allReceiptsResolve = resolve)
    )
    ...
    const start = Date.now()
    console.log(
      `Start sending messages at ${new Date(start).toLocaleString('en-US')}`
    )
    const delayPromise = delay(period).then(() => undefined)
    await sendConcurrently(activeAgents, channel, messagesToSend, 'signal')
    console.log(
      `Finished sending messages at ${new Date(Date.now()).toLocaleString(
        'en-US'
      )}`
    )
    console.log(`Getting messages (should be ${messagesToSend})`)
  
    const finishTime: number | undefined = await Promise.race([
      allReceipts,
      delayPromise
    ])
    ...
    if (finishTime === undefined) {
      ...
      console.log(
        `Total signals received: ${totalReceived} (${(
      ...
      console.log(
        `Peers amongst active agents: Min: ${min} Max: ${max} Avg ${avg}`
      )
      return undefined
    }
  ...
  const sendOnInterval = async (
    ...
    do {
      ...
      if (intervalDuration < sendInterval) {
        console.log(
          `Waiting ${sendInterval - intervalDuration}ms before sending again`
        )
  ...
  const phaseTrial = async (
    ...
    const peerConsistencyTook = await waitActivePeers( // 'N/A'
    ...
    const activationConsistencyTook = await _waitAgentsActivated(activeAgents) // 'N/A'
    ...
    for (let i = 0; i < allPlayers.length; i++) {
      ...
      conductor.setSignalHandler(signal => {
        ...
        const now = Date.now()
        const latency =
          now - payload.signal_payload.messageData.createdAt[0] * 1000
        let tranch: number
        if (latency < 5000) {
          tranch = 5000
        ...
        console.log(
          `${key} got signal with latency ${latency}. Total so far: ${totalSignalsReceived}`
        )
      })
    }
    ...
    console.log(
      `   1 message per ${senders} sender ${sendInterval}ms for ${period}ms`
    )
    const totalMessagesSent = await sendOnInterval(
      senders,
      activeAgents,
      channel,
      period,
    ...
    do {
      console.log(
        `Waiting for ${msToWaitBeforeCallingItQuits /
          1000}s for signals to finish arriving (expecting ${totalSignalsExpected})`
      )
    ...
    console.log(
      `Waiting for signals ends at ${new Date(waitingEnd).toLocaleString(
        'en-US'
      )}`
    )
    ...
    while (!done) {
      const results = await doListMessages(
        `listMessages at min ${listMessagesMin} after waiting for signals`,
        channel,
        activeAgents
      )
    ...
    _took(
      `Waiting for ${PEER_CONSISTENCY_PERCENT}% peer consistency`,
      peerConsistencyTook
    )
    _took('Waiting for active agent consistency', activationConsistencyTook)
    console.log(`Senders:	${senders}`)
    console.log(
      `Sending 1 message per ${senders} sender ${(sendInterval / 1000).toFixed(
        1
      )}s for ${(period / 1000).toFixed(1)}s`
    )
    took(`Sending ${totalMessagesSent} messages`, start, phaseEnd)
    ...
    took(
      `From start of sending till fished waiting for signals`,
      start,
      waitingEnd
    )
  
    console.log(
      `Total signals received:	${totalSignalsReceived} (${(
        (totalSignalsReceived / totalSignalsExpected) *
        100
      ).toFixed(2)}%)`
    )
    ...
    console.log(
      `Final peers count in peer stores of active peers:\nMin:	${m.min}\nMax:	${
        m.max
      }\nAvg:	${m.avg.toFixed(1)}`
    )
    ...
    for (let i = 0; i < listMessagesResults.length; i++) {
      logCounts(
        `listMessages at min ${i}`,
        listMessagesResults[i],
        totalMessagesSent
      )
    }
  }
  ...
  const logCounts = (msg, messagesByAgent, totalMessages) => {
    ...
    for (const c of messagesByAgent) {
      let tranch: string
      if (c < totalMessages * 0.25) {
        tranch = '0-25%'
      } else if (c < totalMessages * 0.75) {
        tranch = '25-75%'
      } else if (c < totalMessages * 0.99) {
        tranch = '75-99%'
      } else {
        tranch = ' 100%'
      }
  ...
  export const signalTx = async (s, t, config, period, txCount, local) => {
    ...
    const actual = await signalTrial(
      period,
      activeAgents,
      allPlayers,
      channel,
