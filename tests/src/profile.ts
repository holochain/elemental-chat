...
module.exports = (orchestrator) => {
	orchestrator.registerScenario('test profile zomes', async (s, t) => {
		...
		const jcHapp = await installJCHapp(
			(
				await s.players([localConductorConfig])
			)[0]
		)

		const [alice_chat_happ, bob_chat_happ] = await installAgents(
			conductor,
			['alice', 'bobbo'],
			jcHapp
		)
		...
		let bobbo_check_alice_profile = await bobbo.call(
			'profile',
			'get_profile',
			a_check_a_profile.agent_address
		)
		...
		bobbo_check_alice_profile = await bobbo.call(
			'profile',
			'get_profile',
			a_check_a_profile.agent_address
		)
